...
-- Disable PgUp & PgDn (Very inconviniently placed on the T14)
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      local function live_grep_git_root()
        local git_root = "~/git/ClientPortal/"
        if git_root then
      ...
      --vim.keymap.set('n', '<leader>sg', builtin.git_files, { desc = '[S]earch Git by [g]' })
      vim.keymap.set('n', '<leader>sG', builtin.live_grep, { desc = '[S]earch by [G]rep' })
      vim.keymap.set('n', '<leader>sd', builtin.diagnostics, { desc = '[S]earch [D]iagnostics' })
      vim.keymap.set('n', '<leader>sr', builtin.resume, { desc = '[S]earch [R]esume' })
      vim.keymap.set('n', '<leader>s.', builtin.oldfiles, { desc = '[S]earch Recent Files ("." for repeat)' })
      vim.keymap.set('n', '<leader><leader>', builtin.buffers, { desc = '[ ] Find existing buffers' })
      vim.keymap.set("n", "<leader>sg", ":LiveGrepGitRoot<cr>", { desc = "[S]earcg by [g]rep on Git Root" })
